...
import copy
import os
from parallel_circuit_cycle import calculate_parallel_circuit_cycle
...
def optimize_parallel_circuit_cycle(input_values, y):
    ...
    input_values_x = copy.copy(input_values)
    cycle=calculate_parallel_circuit_cycle(input_values_x)
...
def optimize_parallel_circuit_cycle_with_multiple_refrigerants(input_values, y ,input_ranges):
    ...
    results = pd.DataFrame(columns=[
        ...
        'Resfriamento no trocador (geladeira)',
        'Resfriamento no trocador (freezer)',        
        'x5',
        'x8'])
    n = 0
    print('Starting')
    for refrigerant in input_ranges['refrigerants']:
        for t_external in input_ranges['t_external']:
            ...
            results = results.append({
                ...
                'Resfriamento no trocador (geladeira)':optimized_cycle['cooling_FFC'],
                'Resfriamento no trocador (freezer)':optimized_cycle['cooling_FZC'],
...
optimized_exergy_efficiency_table = optimize_parallel_circuit_cycle_with_multiple_refrigerants(input_values, 'cop', input_ranges)

# Create a Pandas Excel writer using XlsxWriter as the engine
writer = pd.ExcelWriter('parallel_cycle_optimization.xlsx', engine='xlsxwriter')
